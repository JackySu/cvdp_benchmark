...
def add_license_network_to_docker_compose(
    content: str, license_network_name: str
) -> str:
    """
    ...
    try:
        ...
        data["networks"][license_network_name].update(
            {"name": license_network_name, "external": True}
        )
            ...
            if "networks" in service_config:
                ...
                if isinstance(service_config["networks"], list):
                    ...
                    for i, network in enumerate(networks):
                        if (
                            network == "licnetwork"
                            and license_network_name != "licnetwork"
                        ):
                            networks[i] = license_network_name
                elif isinstance(service_config["networks"], dict):
                    # Handle dict format: {default: {}, licnetwork: {}}
                    if (
                        "licnetwork" in service_config["networks"]
                        and license_network_name != "licnetwork"
                    ):
                        service_config["networks"][license_network_name] = (
                            service_config["networks"].pop("licnetwork")
                        )
...
class Repository:
    ...
    def restore_files(self, files):
        for file, content in files.items():
            ...
            # Fix outdated cocotb in Dockerfile - upgrade to cocotb 1.9.2 for Xcelium compatibility
            # cocotb 1.9.x has -NEW_VHPI_PROPAGATE_DELAY flag that's incompatible with Xcelium < 23.09
            if file == "Dockerfile" or file.endswith("/Dockerfile"):
                # Check if this Dockerfile uses pip/pip3 and doesn't already install cocotb==1.9.2
                if (
                    ("pip install" in content or "pip3 install" in content)
                    and "cocotb>=" not in content
                    and "cocotb==" not in content
                ):
                    # Upgrade to cocotb 1.9.2 which fixes Xcelium compatibility issues
                    import re

                    updated = False

                    # Pattern 1: Match "RUN pip/pip3 install cocotb_bus" or "cocotb-bus"
                    pattern1 = r"RUN (pip3?) install (cocotb[-_]bus)"
                    replacement1 = r'RUN \1 install "cocotb==1.9.2" \2'
                    new_content = re.sub(pattern1, replacement1, content)
                    if new_content != content:
                        content = new_content
                        updated = True

                    # Pattern 2: Match "RUN pip/pip3 install cocotb" (standalone, not cocotb_bus/cocotb-bus)
                    # Only apply if Pattern 1 didn't match
                    if not updated:
                        pattern2 = r"RUN (pip3?) install cocotb\b(?![-_])"
                        replacement2 = r'RUN \1 install "cocotb==1.9.2"'
                        new_content = re.sub(pattern2, replacement2, content)
                        if new_content != content:
                            content = new_content
                            updated = True

                    # Add sed patch to remove -NEW_VHPI_PROPAGATE_DELAY flag (only supported in Xcelium >= 23.09)
                    # This applies PR #4085 fix for Xcelium < 23.09 compatibility
                    if updated and "NEW_VHPI_PROPAGATE_DELAY" not in content:
                        # Find the cocotb install line and append the sed patch
                        sed_patch = r' && sed -i "s/vhpi_opts.append.*NEW_VHPI_PROPAGATE_DELAY.*/#\\0  # Patched for Xcelium < 23.09/" $(python3 -c "import cocotb,os;print(os.path.join(os.path.dirname(cocotb.__file__),'"'"'runner.py'"'"'))")'
                        content = re.sub(
                            r'(RUN pip3? install "cocotb==1\.9\.2")',
                            r'\1' + sed_patch,
                            content
                        )
                    ...
                    if updated:
                        print(
                            f"Updated Dockerfile to install cocotb 1.9.2 for Xcelium compatibility"
                        )

            # Fix coverage report path in harness_library.py
            # The imc command writes coverage.log to cwd, but covt_report_check() looks for /code/rundir/coverage.log
            if file.endswith(".py") and "-out coverage.log" in content:
                content = content.replace("-out coverage.log", "-out /code/rundir/coverage.log")
                print(f"Updated {file} to use absolute path for coverage.log")

            # Fix coverage_report() to properly check imc command result and print errors
            if file.endswith(".py") and "assert(subprocess.run(cmd, shell=True))" in content:
                content = content.replace(
                    "assert(subprocess.run(cmd, shell=True)), \"Coverage merge didn't ran correctly.\"",
                    "result = subprocess.run(cmd, shell=True, capture_output=True, text=True); "
                    "print(f'imc stdout: {result.stdout}'); print(f'imc stderr: {result.stderr}'); "
                    "assert result.returncode == 0, f\"Coverage merge failed with code {result.returncode}: {result.stderr}\""
                )
                print(f"Updated {file} to properly check imc command result")
            ...
            if self.requires_eda_license and file.endswith("docker-compose.yml"):
                ...
                if license_network_name:
                    content = add_license_network_to_docker_compose(
                        content, license_network_name
                    )
                        ...
                        for service_name, service_config in data["services"].items():
                            if "volumes" in service_config:
                                ...
                                for volume in service_config["volumes"]:
                                    if "/code" in volume:
                                        print(
                                            f"Warning: Removing '/code' volume mapping: {volume} in issue '{self.name}' (id {self.id})"
                                        )
                                    else:
    ...
    def exec_timeout(
        self, cmd, kill=None, out=None, monitor_dir=None, monitor_kill_cmd=None
    ):
        # ----------------------------------------
        # - Args
        # ----------------------------------------
        ...
        if monitor_dir is not None and hasattr(self, "dir_monitor"):
            ...
            self.dir_monitor.start_monitoring(
                directory=monitor_dir, process_id=pid, kill_cmd=monitor_kill_cmd or kill
            )
    ...
    def log_run(
        self, cmd, kill=None, logfile="", monitor_dir=None, monitor_kill_cmd=None
    ):
        if self.debug:
            print(f"{cmd}")
        ...
        if logfile != "":
            with open(logfile, "w+") as out:
                returncode, pid = self.exec_timeout(
                    cmd, kill, out, monitor_dir, monitor_kill_cmd
                )
        else:
            returncode, pid = self.exec_timeout(
                cmd, kill, None, monitor_dir, monitor_kill_cmd
            )
    ...
    def log_docker(
        ...
        # Otherwise, ensure docker-compose file has network configuration before proceeding
            ...
            try:
                ...
                print(
                    f"Warning: Failed to add network configuration to {docker}: {str(e)}"
                )
        ...
        with open(script_path, "w") as script_file:
            ...
            script_file.write(
                f"# Auto-generated script to run harness Docker container\n"
            )
            script_file.write(
            ...
            script_file.write(
                f"  docker compose -f {docker} -p {project_name} kill {service} 2>/dev/null || true\n"
            )

            # Cleanup image
            script_file.write(
                f"  docker rmi {project_name}-{service} 2>/dev/null || true\n"
            )
            ...
            script_file.write(
                f'echo "Running harness with project name: {project_name}"\n'
            )
            ...
            script_file.write(
                f"  docker compose -f {docker} -p {project_name} run --rm --user $USER_ID:$GROUP_ID -e HOME=/code/rundir --entrypoint bash {cmd} {service}\n"
            )
            script_file.write(f"else\n")
            script_file.write(
                f"  docker compose -f {docker} -p {project_name} run --rm --user $USER_ID:$GROUP_ID -e HOME=/code/rundir {cmd} {service}\n"
            )
            script_file.write(f"fi\n")
            ...
            script_file.write(
                f"# Append build logs if they exist (contains actual compiler/tool errors)\n"
            )
            script_file.write(f'SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"\n')
            script_file.write(
                f'for logfile in "$SCRIPT_DIR/rundir/sim.log" "$SCRIPT_DIR/rundir/sim_build/sim.log" "$SCRIPT_DIR/rundir/build.log"; do\n'
            )
    ...
    def create_workspace_volume_script(
        self, docker_dir, repo_url=None, commit_hash=None, patches=None, root_dir=None
    ):
        """
        ...
        with open(create_script_path, "w") as script_file:
            ...
            script_file.write(
                f"if docker volume inspect {volume_name} &>/dev/null; then\n"
            )
            script_file.write(
                f'  echo "Volume {volume_name} already exists. Removing it first..."\n'
            )
            ...
            if repo_url and commit_hash:
                ...
                script_file.write(
                    "# Find the mirror directory (should be named with repo hash)\n"
                )
                ...
                script_file.write(
                    '  echo "ERROR: No git cache mirror found in $GIT_CACHE_DIR"\n'
                )
                script_file.write(
                ...
                script_file.write(
                    "FROM registry.cn-shanghai.aliyuncs.com/icb/ubuntu:24.04\n"
                )
        ...
        with open(destroy_script_path, "w") as script_file:
            ...
            script_file.write(
                f'  echo "Volume {volume_name} does not exist. Nothing to destroy."\n'
            )
        ...
        try:
            ...
            if self.sbj_llm_model and category not in BLEU_SCORING_CATEGORIES:
                ...
                result.append(
                    {
                        "result": err,
                        "log": f"{logfile}_llm_score.txt",
                        "error_msg": None,
                        "execution": llm_time - start_time,
                        "llm_score": llm_score,
                    }
                )
            else:
                ...
                if category in BLEU_SCORING_CATEGORIES:
                    result.append(
                        {
                            "result": err,
                            "log": f"{logfile}_rouge.txt",
                            "error_msg": None,
                            "execution": rouge_time - start_time,
                        }
                    )
                    result.append(
                        {
                            "result": err,
                            "log": f"{logfile}_bleu.txt",
                            "error_msg": None,
                            "execution": bleu_time - rouge_time,
                            "bleu_score": bleu,
                        }
                    )
                else:
            ...
            result = [
                {
                    "result": 1,
                    "log": None,
                    "error_msg": f"Failed to execute subjective tests: {str(e)}",
                    "execution": time.time() - start_time,
                }
            ]
            err = 3
    ...
    def subjective_score(self, response, reference, problem_prompt: str = ""):
        ...
        try:
            ...
            score = self.sbj_llm_model.subjective_score(
                response, reference, problem_prompt
            )

            if self.debug:
                print(
                    f"LLM-based subjective score: {score}/1.0 (threshold: {self.llm_score_th})"
                )
            return score
            # else:
    ...
    def obj_harness(self, issue_path: str = "", logfile: str = "", uut: str = None):
        ...
        if os.path.exists(docker):
            ...
            for i, service in enumerate(services):
                ...
                service_log = (
                    f"{logfile}{f'_{service}' if len(services) > 1 else ''}.txt"
                )
                ...
                [
                    {
                        "result": 1,
                        "log": None,
                        "error_msg": "Failed to execute objective harness",
                        "execution": 0,
                    }
                ],
                1,
    ...
    def clean_up(self):
        ...
        if self.volume_name:
            try:
                ...
                print(
                    f"[WARNING] Failed to cleanup workspace volume {self.volume_name}: {e}"
                )
