...
config.register_config(
    "LLM_RETRY_COUNT",
    default=LLM_RETRY_COUNT_DEFAULT,
    type_cast=int,
...
if QUEUE_TIMEOUT is not None:
    ...
    print(
        "Queue timeout is disabled by default. Set QUEUE_TIMEOUT environment variable to enable it."
    )
...
class ThreadingTimeout:
    ...
    def _timeout_function(self):
        ...
        self.exc_info = TimeoutError(
            f"Function call timed out after {self.seconds} seconds"
        )
...
"""
    ...
    def _create_subjective_model(self, model_config=None):
        ...
        try:
            ...
            logging.error(
                f"Failed to create subjective scoring model '{model_config}': {str(e)}"
            )
            return None
    ...
    def configure_subjective_scoring(self, model_config=None, threshold=None):
        ...
        logging.info(
            f"Subjective scoring configured: model={self._sbj_model_config}, threshold={self._sbj_scoring_threshold}"
        )
    ...
    def get_context_for_repo(self, id, model):
        ...
        raise NotImplementedError(
            "get_context_for_repo must be implemented by subclasses"
        )
    ...
    def create_repo(self, id: str, model: OpenAI_Instance = None):
        ...
        if not id in self.files:
            print(
                f"Creating harness environment for datapoint: {name}_{str(issue).zfill(4)}"
            )
    ...
    def set_repo(self, id: str, context={}):
        ...
        if not id in self.files:
            print(
                f"Recreating harness environment for datapoint: {name}_{str(issue).zfill(4)}"
            )
    ...
    def run_subjective_scoring(self, id, repo, model=None, obj=False):
        ...
        if "subjective_reference" in self.context[id]:
            ...
            reference = ""
            logging.error(
                f"ERROR: No reference response found for {id}. Subjective scoring will be inaccurate and likely fail!"
            )
        ...
        if "subjective.txt" in self.files[id]:
            ...
            print(
                f"[WARNING] No content found for subjective scoring of {id}. Skipping evaluation..."
            )
            tests = [
                {
                    "result": 1,
                    "log": None,
                    "error_msg": "No content for subjective scoring",
                    "execution": 0.0,
                }
            ]
            return tests, 1
        ...
        if not uut or len(uut) == 0:
            print(
                f"[WARNING] Empty content for {id}. Skipping subjective evaluation..."
            )
            tests = [
                {
                    "result": 1,
                    "log": None,
                    "error_msg": "Empty content for subjective scoring",
                    "execution": 0.0,
                }
            ]
            return tests, 1
                    ...
                    q.put(
                        {
                            "id": id,
                            "refined": False,
                            "error": "No refinement model provided",
                        }
                    )
                return
    ...
    def all_refine(self, model_factory=None):
        ...
        if not model_factory:
            ...
            return {
                ...
                "total": 0,
                "errors": 1,
                "error": "No model factory provided",
            }
        ...
        if not hasattr(self, "context") or not self.context:
            ...
            return {
                ...
                "total": 0,
                "errors": 1,
                "error": "No datapoints loaded",
            }

        # Create a single refinement model for all threads to use
        try:
            print(f"Creating refinement model: {self.refine_model}")
            refine_model = model_factory.create_model(
                model_name=self.refine_model, context=self.folders
            )

            if not hasattr(refine_model, "refine") or not callable(refine_model.refine):
                print(
                    f"Error: Model '{self.refine_model}' does not support the refine() method"
                )
                return {
                    "refined": 0,
                    "total": 0,
                    "errors": 1,
                    "error": f"Model {self.refine_model} does not support refine()",
                }
        except Exception as e:
        ...
        print(
            f"Found {len(to_refine)} datapoints to refine out of {len(self.context)} total"
        )
        ...
        def process_refine_results(result_queue, task_queue, expected_count):
            ...
            print(
                f"Refinement complete: {refined_count} datapoints refined, {error_count} errors"
            )
    ...
    def _try_refine_datapoint(self, id, model, repo):
        ...
        try:
            ...
            refinement_context = {"datapoint": original_datapoint, "id": id}
            ...
            if self._validate_refined_datapoint(refined_datapoint, id):
                refined_datapoint["original_prompt"] = original_datapoint["input"][
                    "prompt"
                ]
    ...
    def th_prepare(self, id, model):
        ...
        try:
            ...
            self.runs[id] = {"obj": obj, "repo": repo, "input": input, "output": diff}
    ...
    def th_run(self, id, q: queue.Queue = None, model: OpenAI_Instance = None):
        ...
        try:
            ...
            res = self.run(
                id=id, obj=self.runs[id]["obj"], repo=self.runs[id]["repo"], model=model
            )
                ...
                if "tests" in res:
                    ...
                    for test in res["tests"]:
                        if (
                            "error_msg" in test
                            and test["error_msg"]
                            and "agent_error" in test["error_msg"]
                        ):
    ...
    def all_run(self, model: OpenAI_Instance = None):
        ...
        if not self.runs:
            raise RuntimeError(
                "Cannot start execution phase: Preparation phase has not been completed"
            )
        ...
        result = executor.execute_parallel_with_results(
            ...
            task_args=[
                model
            ],  # Note: result_queue will be inserted as second arg by executor
...
class CopilotProcessor(DatasetProcessor):
    def __init__(
        ...
        super().__init__(
            filename, golden, threads, debug, host, prefix, network_name, manage_network
        )
    ...
    def create_context(self, id: str, model=None):
        ...
        if self.golden:
            if not self.disable_patch:
                ...
                if cat in CODE_COMPREHENSION_CATEGORIES:
                    ...
                    if (
                        "response" in context["output"]
                        and context["output"]["response"]
                    ):
        ...
        # ----------------------------------------
            ...
            if (
                ...
                and isinstance(context["categories"][0], str)
                and context["categories"][0].startswith("cid")
            ):
                cat = int(context["categories"][0][3:])
            ...
            while 1:
                ...
                try:
                    if model != None:
                        cat = None
                        if (
                            "categories" in self.context[id]
                            and self.context[id]["categories"]
                            and isinstance(self.context[id]["categories"][0], str)
                            and self.context[id]["categories"][0].startswith("cid")
                        ):
                            cat = int(self.context[id]["categories"][0][3:])
                        ...
                        if (
                            ...
                            cat = int(self.context[id]["categories"][0][3:])
                        output, success = self.model.prompt(
                            prompt,
                            schema=schema_to_use,
                            prompt_log=logfile,
                            files=files,
                        ...
                        raise ValueError(
                            "Unable to execute harness without an model assigned."
                        )
                    ...
                    if not success:
                        if llm_retry_count > 0:
                            ...
                            logging.error(
                                "Failed to parse JSON response after multiple retries"
                            )
                            output = {}
                            break
                    else:
    ...
    def _validate_refined_datapoint(self, refined_datapoint, original_id):
        ...
        if "prompt" in refined_datapoint:
            ...
            if (
                "ambiguity_score" not in refined_datapoint
                or "consistency_score" not in refined_datapoint
            ):
            ...
            if "ambiguity_score" in refined_datapoint:
                original_datapoint["ambiguity_score"] = refined_datapoint[
                    "ambiguity_score"
                ]

            if "consistency_score" in refined_datapoint:
                original_datapoint["consistency_score"] = refined_datapoint[
                    "consistency_score"
                ]
        ...
        if (
            not isinstance(refined_datapoint["input"], dict)
            or "context" not in refined_datapoint["input"]
        ):
    ...
    def _save_refined_datapoints(self):
        ...
        if not self.refined_filename:
            ...
            self.refined_filename = os.path.join(
                dirname, f"{os.path.splitext(basename)[0]}_refined.jsonl"
            )
        ...
        if ambiguity_scores:
            refinement_stats["avg_ambiguity"] = sum(ambiguity_scores) / len(
                ambiguity_scores
            )
        if consistency_scores:
            refinement_stats["avg_consistency"] = sum(consistency_scores) / len(
                consistency_scores
            )

        print(
            f"Saved {refinement_stats['total']} refined datapoints to {self.refined_filename}"
        )

        if refinement_stats["with_scores"] > 0:
            print(f"Refinement statistics:")
            print(
                f"  - Datapoints with scores: {refinement_stats['with_scores']}/{refinement_stats['total']}"
            )
            print(
                f"  - Average ambiguity score: {refinement_stats['avg_ambiguity']:.2f}/10"
            )
            print(
                f"  - Average consistency score: {refinement_stats['avg_consistency']:.2f}/10"
            )
...
class AgenticProcessor(DatasetProcessor):
    ...
    def __init__(
        ...
        result = subprocess.run(
            ["docker", "images", "-q", "patch_image"], capture_output=True, text=True
        )
        ...
        if not result.stdout.strip():
            ...
            with open(dockerfile, "w") as f:
                f.write("FROM registry.cn-shanghai.aliyuncs.com/icb/ubuntu:24.04\nRUN apt update && apt install -y git")

            # Build image
            subprocess.run(
                ["docker", "build", "-t", "patch_image", "-f", dockerfile, "."],
                check=True,
            )
    ...
    def create_repository(
        self, id: str, harness=None, name: str = "", issue: str = "", patches=None
    ):
        # Determine if this specific datapoint requires EDA license network
        from src import commercial_eda
    ...
    def create_context(self, id: str, model: OpenAI_Instance = None):
        ...
        if id in self.agent_results:
            ...
            return self.result_context(
                int(name[-1]), context, self.context[id]["patch"]
            )

    def agent_run(
        self, issue_path: str = "", agent: str = "", monitor_size=True, **kargs
    ):
        ...
        if hasattr(self, "include_golden_patch") and self.include_golden_patch:
            ...
            if data_id and "patch" in self.context[data_id]:
                ...
                golden_patch_file = os.path.join(
                    issue_path, "golden_ref_solution.patch"
                )
                ...
                if is_context_heavy and (repo_url and commit_hash):
                    ...
                    for _, data in self.runs.items():
                        if "repo" in data and data["repo"] is not None:
                            if (
                                hasattr(data["repo"], "issue_path")
                                and data["repo"].issue_path == issue_path
                            ):
                                if (
                                    hasattr(data["repo"], "volume_name")
                                    and data["repo"].volume_name
                                ):
        ...
        if use_git_workspace and workspace_volume:
            ...
            docker_compose = {
                ...
                "volumes": {
                    workspace_volume: {"external": True, "name": workspace_volume}
                },
            }
        else:
        ...
        if hasattr(self, "include_golden_patch") and self.include_golden_patch:
            ...
            if os.path.exists(golden_patch_file):
                docker_compose["services"]["agent"]["volumes"].append(
                    "./golden_ref_solution.patch:/code/golden_ref_solution.patch"
                )
                print(
                    "Golden patch file will be mounted in the agent container at /code/golden_ref_solution.patch"
                )
        ...
        if hasattr(self, "include_harness") and self.include_harness:
            ...
            if os.path.exists(src_dir):
                ...
                print(
                    "Test harness src directory will be mounted in the agent container at /code/src"
                )
            ...
            if os.path.exists(docker_compose_yml):
                ...
                print(
                    "Original docker-compose.yml will be mounted in the agent container at /code/docker-compose.yml"
                )
        ...
        if hasattr(self, "network_name") and self.network_name:
            docker_compose["networks"] = {
                "default": {"name": self.network_name, "external": True}
            }
            ...
            if commercial_eda.datapoint_requires_eda_license(datapoint):
                ...
                if license_network_name:
                    ...
                    if "networks" not in docker_compose["services"]["agent"]:
                        ...
                        docker_compose["services"]["agent"]["networks"], list
                    ):
                    ...
                    if (
                        license_network_name
                        not in docker_compose["services"]["agent"]["networks"]
                    ):
                        docker_compose["services"]["agent"]["networks"].append(
                            license_network_name
                        )

                    print(
                        f"Added license network '{license_network_name}' to agent Docker Compose for commercial EDA datapoint"
                    )
        ...
        before_volume = None
        if use_git_workspace and workspace_volume:
            before_volume = self._create_before_snapshot_volume(
                workspace_volume, issue_path
            )
        ...
        formatted_repo = "".join(
            c.lower() if c.isalnum() or c == "-" or c == "_" else "_" for c in repo_name
        )
        ...
        if repo_instance is not None:
            ...
            reports_dir = os.path.join(
                os.path.dirname(os.path.dirname(issue_path)), "reports"
            )
        ...
        script_path = os.path.join(
            os.path.dirname(docker_compose_path), "run_docker_agent.sh"
        )
        ...
        try:
            ...
            kill_cmd = (
                f"docker compose -f {docker_compose_path} -p {project_name} kill agent"
            )
            ...
            with open(logfile, "w") as log_file:
                ...
                p = subprocess.Popen(
                    script_path, shell=True, stdout=log_file, stderr=subprocess.STDOUT
                )
                pid = p.pid
                ...
                if monitor_size:
                    ...
                    self.dir_monitor.start_monitoring(
                        directory=issue_path, process_id=pid, kill_cmd=kill_cmd
                    )
                ...
                try:
                    ...
                    print(
                        f"Timeout for {script_path} ({repository.DOCKER_TIMEOUT}s) expired"
                    )
            ...
            if (not hasattr(self, "network_name") or not self.network_name) and (
                not hasattr(self, "manage_network") or self.manage_network
            ):
                try:
            ...
            if use_git_workspace and workspace_volume and before_volume:
                try:
                    self._generate_volume_changes_patch(
                        workspace_volume, before_volume, issue_path
                    )
                except Exception as e:
                    ...
                    try:
                        subprocess.run(
                            ["docker", "volume", "rm", "-f", before_volume],
                            check=False,
                            capture_output=True,
                        )
                    except Exception as cleanup_e:
                        print(
                            f"[WARNING] Failed to cleanup before volume {before_volume}: {cleanup_e}"
                        )
    ...
    def create_agent_script(self, docker_compose_path, project_name):
        ...
        if hasattr(self, "network_name") and self.network_name:
            ...
            try:
                network_util.add_network_to_docker_compose(
                    docker_compose_path, self.network_name
                )
            except Exception as e:
                print(
                    f"Warning: Failed to add network configuration to {docker_compose_path}: {str(e)}"
                )
        ...
        with open(script_path, "w") as script_file:
            ...
            script_file.write(
                f"# Auto-generated script to run agent Docker container\n"
            )
            script_file.write(
                f"# Usage: {os.path.basename(script_path)} [-d] (where -d enables debug mode with bash entrypoint)\n"
            )
            ...
            if hasattr(self, "network_name") and self.network_name:
                ...
                script_file.write(
                    f"if ! docker network inspect {self.network_name} &>/dev/null; then\n"
                )
                script_file.write(
                    f'  echo "Creating Docker network {self.network_name}..."\n'
                )
                script_file.write(
            ...
            # Cleanup image
            script_file.write(
                f"  docker rmi {project_name}-agent 2>/dev/null || true\n"
            )
            ...
            if hasattr(self, "network_name") and self.network_name:
                ...
                script_file.write(
                    f'    echo "Removing Docker network {self.network_name}..."\n'
                )
                script_file.write(
                    f"    docker network rm {self.network_name} 2>/dev/null || true\n"
                )
                script_file.write(f"  fi\n")
            else:
                # Use more robust filtering approach for default networks
                script_file.write(
                    f"  docker network ls --filter name={project_prefix} -q | xargs -r docker network rm 2>/dev/null || true\n"
                )
            ...
            script_file.write(
                f'echo "Running agent with project name: {project_name}"\n'
            )
            ...
            script_file.write(
                f'  echo "DEBUG MODE: Starting container with bash entrypoint"\n'
            )
            script_file.write(
                f"  docker compose -f {docker_compose_path} -p {project_name} run --rm --user $USER_ID:$GROUP_ID --entrypoint bash agent\n"
            )
            script_file.write(f"else\n")
            script_file.write(
                f"  docker compose -f {docker_compose_path} -p {project_name} run --rm --user $USER_ID:$GROUP_ID agent\n"
            )
            script_file.write(f"fi\n")
    ...
    def th_agent(self, id):
        ...
        try:
            ...
            is_context_heavy = (
                has_agentic_heavy and bool(repo_url) and bool(commit_hash)
            )
            ...
            if is_context_heavy:
                if not self.golden:
                    ...
                    if agent_status != 0:
                        error_msg = (
                            f"Agent process exited with non-zero status: {agent_status}"
                        )
                ...
                if not self.golden:
                    ...
                    if self._is_mi6_agent(self.agent):
                        ...
                        # Prepare mi6 agent files (doc, files list, sim script)
                        self._prepare_mi6_agent_files(issue_path, id, target_files)

                        # Run agent once per target file
                        for target_file in target_files:
                            print(f"Running mi6 agent for target file: {target_file}")

                            # Create mi6 prompt.json with task, doc, files for this specific file
                            self._create_mi6_prompt_json(
                                issue_path, base_prompt, target_file, context_data
                            )

                            # Run agent
                            agent_status, agent_logfile = self.agent_run(
                                issue_path, self.agent
                            )
                            result["agent_logfile"] = agent_logfile
                            ...
                            if agent_status != 0:
                                ...
                                logging.error(error_msg)
                                result["agent_error"] = (
                                    result.get("agent_error", "") + error_msg + "\n"
                                )
                                continue

                            # Handle mi6 output: move file and generate diff
                            content, diff = self._handle_mi6_output(
                                issue_path, target_file, before_dir
                            )
                            ...
                            if content:
                                ...
                                result[f"file_error_{target_file}"] = (
                                    f"Failed to process mi6 output for {target_file}"
                                )
                        ...
                        try:
                            ...
                            harness_dir = os.path.dirname(issue_path)
                            repo_name_base = os.path.basename(
                                os.path.dirname(harness_dir)
                            )
                        ...
                        for d in dir_names:
                            ...
                            orig_files = {
                                f: os.path.join(orig_dir, f)
                                for f in self._get_files(orig_dir)
                            }
                            mod_files = {
                                f: os.path.join(mod_dir, f)
                                for f in self._get_files(mod_dir)
                            }
                                ...
                                try:
                                    ...
                                    if rel_path not in orig_files:
                                        ...
                                        if orig_content != mod_content:
                                            ...
                                            diff = self._diff(
                                                orig_content, mod_content, context_path
                                            )
                                            unified_patch.append(diff)
                                            unified_patch.append(
                                                ""
                                    ...
                                    file_error = (
                                        f"Error processing {mod_path}: {str(file_e)}"
                                    )
    ...
    def all_agent(self):
        ...
        executor = ParallelExecutor(
            num_workers=self.threads, phase_name="Agent Processing"
        )
        executor.execute_parallel_simple(
            task_func=self.th_agent, items=list(self.context.keys())
        )
    ...
    def _handle_mi6_output(
        self, issue_path: str, target_path: str, before_dir: str
    ) -> tuple:
        """
        ...
        possible_outputs = [
            os.path.join(issue_path, "src", "out.sv"),
            os.path.join(issue_path, "out.sv"),
            os.path.join(issue_path, target_path),  # mi6 might write directly to target
        ]
        ...
        if not mi6_output:
            logging.error(f"mi6 output file not found in any expected location: {possible_outputs}")
            return None, None
        ...
        # Move generated file to target location (if not already there)
        if mi6_output != full_target_path:
            shutil.move(mi6_output, full_target_path)
            print(f"Moved mi6 output from {mi6_output} to: {full_target_path}")
        else:
            print(f"mi6 output already at target: {full_target_path}")

        # Generate diff using GNU diff
        diff_content = self._generate_gnu_diff(
            original_content, generated_content, target_path
        )
    ...
    def _create_mi6_prompt_for_file(
        self, base_prompt: str, target_file: str, context_data: dict = None
    ) -> str:
        """
    ...
    def _extract_interface_from_context(
        self, context_data: dict, target_file: str
    ) -> str:
        """
        Extract expected module interface from context files.

        Checks multiple sources:
        1. Original file in context (if modifying existing file)
        2. Testbench instantiation
        ...
        # Get module name from target file (e.g., 'rtl/aes_ke.sv' -> 'aes_ke')
        module_name = os.path.splitext(os.path.basename(target_file))[0]
        interface_info = ""

        # Method 1: Check if the original file exists in context - extract its interface
        if target_file in context_data and context_data[target_file]:
            original_content = context_data[target_file]
            # Extract module declaration
            module_pattern = (
                rf"module\s+{module_name}\s*(?:#\s*\([^)]*\))?\s*\(([^;]*?)\);"
            )
            match = re.search(module_pattern, original_content, re.DOTALL)
            if match:
                port_declaration = match.group(0)
                interface_info = f"CRITICAL: You are MODIFYING an existing module. You MUST preserve the EXACT same interface.\n"
                interface_info += f"The original module declaration is:\n```systemverilog\n{port_declaration}\n```\n"
                interface_info += f"\nDO NOT change port names, widths, or directions. Only modify the internal logic."
                return interface_info

        # Method 2: Look for testbench files in context
        testbench_content = None
        for file_path, content in context_data.items():
            if "verif" in file_path and (
                "_tb.sv" in file_path or "_tb.v" in file_path or "test_" in file_path
            ):
                testbench_content = content
                break

        if testbench_content:
            # Look for module instantiation pattern in testbench
            inst_pattern = (
                rf"{module_name}\s+(?:#\s*\([^)]*\)\s*)?\w+\s*\(([\s\S]*?)\);"
            )
        ...
        # Method 3: Look for instantiation in other RTL files (parent modules)
        for file_path, content in context_data.items():
            if "rtl" in file_path and file_path != target_file and content:
                inst_pattern = (
                    rf"{module_name}\s+(?:#\s*\([^)]*\)\s*)?\w+\s*\(([\s\S]*?)\);"
                )
                match = re.search(inst_pattern, content)
                if match:
                    instantiation = match.group(0)
                    port_pattern = r"\.(\w+)\s*\("
                    ports = re.findall(port_pattern, instantiation)
                    if ports:
                        interface_info = f"CRITICAL: The module '{module_name}' is instantiated in '{file_path}' with these ports:\n"
                        interface_info += f"  Ports: {', '.join(ports)}\n"
                        interface_info += f"\nInstantiation for reference:\n```systemverilog\n{instantiation}\n```\n"
                        interface_info += f"You MUST use these exact port names."
                        return interface_info

        return ""

    def _prepare_mi6_agent_files(self, issue_path: str, id: str, target_files: list):
        """
        Prepare files required by mi6 code agent:
        - spec.txt: specification document
        - files.json: list of RTL files available
        - sim.sh: simulation script

        Args:
            issue_path: Path to the issue directory
            id: Datapoint ID
            target_files: List of target file paths to generate
        """
        context_data = self.context[id].get("context", {})
        harness_data = self.context[id].get("harness", {})

        # 1. Create spec document from docs/ or context
        spec_path = os.path.join(issue_path, "spec.txt")
        spec_content = ""

        # Try to get spec from docs in context
        for file_path, content in context_data.items():
            if "docs/" in file_path or "specification" in file_path.lower():
                spec_content += f"=== {file_path} ===\n{content}\n\n"

        # If no docs found, use the prompt as spec
        if not spec_content:
            spec_content = self.context[id].get("prompt", "")

        with open(spec_path, "w", encoding="utf-8") as f:
            f.write(spec_content)
        print(f"Created spec document: {spec_path}")

        # 2. Create files.json with list of RTL and verification files
        files_json_path = os.path.join(issue_path, "files.json")
        rtl_files = []
        verif_files = []

        # Add existing RTL files from context
        for file_path in context_data.keys():
            if file_path.startswith("rtl/") and (
                file_path.endswith(".sv") or file_path.endswith(".v")
            ):
                rtl_files.append(os.path.join("/code", file_path))

        # Add target files (files to be generated)
        for target_file in target_files:
            full_path = os.path.join("/code", target_file)
            if full_path not in rtl_files:
                rtl_files.append(full_path)

        # Add verification/testbench files from context (needed for simulation)
        for file_path in context_data.keys():
            if file_path.startswith("verif/") and (
                file_path.endswith(".sv") or file_path.endswith(".v")
            ):
                verif_files.append(os.path.join("/code", file_path))

        # Also check harness for verification files
        for file_path in harness_data.keys():
            if file_path.startswith("verif/") and (
                file_path.endswith(".sv") or file_path.endswith(".v")
            ):
                full_path = os.path.join("/code", file_path)
                if full_path not in verif_files:
                    verif_files.append(full_path)

        # Combine: RTL files first, then verification files
        all_files = rtl_files + verif_files

        with open(files_json_path, "w", encoding="utf-8") as f:
            json.dump(all_files, f, indent=2)
        print(f"Created files list: {files_json_path} with {len(rtl_files)} RTL + {len(verif_files)} verif files")

        # 3. Create simulation script
        sim_script_path = os.path.join(issue_path, "sim.sh")
        sim_content = self._create_simulation_script(issue_path, id, harness_data)

        with open(sim_script_path, "w", encoding="utf-8") as f:
            f.write(sim_content)
        os.chmod(sim_script_path, 0o755)
        print(f"Created simulation script: {sim_script_path}")

    def _create_simulation_script(
        self, issue_path: str, id: str, harness_data: dict
    ) -> str:
        """
        Create a simulation script for mi6 agent by extracting the actual
        simulation command from the harness's docker-compose.yml.

        Args:
            issue_path: Path to the issue directory
            id: Datapoint ID
            harness_data: Harness configuration from dataset
        ...
        # Try to extract command from docker-compose.yml
        docker_compose_content = harness_data.get("docker-compose.yml", "")
        env_content = harness_data.get("src/.env", "")
        ...
        if docker_compose_content:
            try:
                import yaml
                compose_data = yaml.safe_load(docker_compose_content)
                if compose_data and "services" in compose_data:
                    # Get the first service (usually xrun, iverilog, etc.)
                    for service_name, service_config in compose_data["services"].items():
                        if "command" in service_config:
                            sim_command = service_config["command"]
                            if "working_dir" in service_config:
                                working_dir = service_config["working_dir"]
                            print(f"Extracted simulation command from {service_name}: {sim_command}")
                            break
            except Exception as e:
        ...
        if env_content:
            script += "# Environment variables from harness\n"
            for line in env_content.strip().split("\n"):
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
                    # Export each environment variable
                    script += f"export {line}\n"
            script += "\n"

        # Create working directory
        script += f"# Create and enter working directory\n"
        script += f"mkdir -p {working_dir}\n"
        script += f"cd {working_dir}\n\n"

        if sim_command:
            # Use the extracted command from docker-compose.yml
            script += f"# Simulation command from docker-compose.yml\n"
            script += f"echo \"Running simulation: {sim_command}\"\n"
            script += f"{sim_command} 2>&1\n"
        else:
            # Fallback: Check for test_runner.py or use generic simulation
            has_test_runner = "src/test_runner.py" in harness_data
            categories = self.context[id].get("categories", [])
            is_commercial = any("commercial" in str(c).lower() for c in categories)

            if has_test_runner:
                # Python-based test runner with pytest
                script += """# Python-based test runner
echo "Running Python test..."
if [ -f /code/src/test_runner.py ]; then
    pytest -s --log-cli-level=INFO /code/src/test_runner.py -v 2>&1
else
...
"""
                ...
                # Commercial EDA simulation (xcelium)
                script += """# Commercial EDA simulation using Xcelium
echo "Running Xcelium simulation..."
if command -v xrun &> /dev/null; then
    xrun -64bit -sv -access +rwc /code/rtl/*.sv /code/verif/*.sv 2>&1
else
    echo "ERROR: Xcelium (xrun) not available"
    exit 1
fi
"""
            else:
                # Default: iverilog simulation
                script += """# Open-source simulation using iverilog
echo "Running iverilog simulation..."
if command -v iverilog &> /dev/null; then
    RTL_FILES=$(find /code/rtl -name "*.sv" -o -name "*.v" 2>/dev/null | tr '\\n' ' ')
    VERIF_FILES=$(find /code/verif -name "*.sv" -o -name "*.v" 2>/dev/null | tr '\\n' ' ')

    if [ -n "$RTL_FILES" ] || [ -n "$VERIF_FILES" ]; then
        iverilog -g2012 -o sim.vvp $RTL_FILES $VERIF_FILES 2>&1
        vvp sim.vvp 2>&1
    else
        echo "No RTL or verification files found"
        exit 1
    fi
else
...
"""
    ...
    def _create_mi6_prompt_json(
        ...
        """
        Create prompt.json for mi6 agent with new format:
        {
            "task": "...",
            "doc": "...",  (optional, can use spec.txt)
            "files": [...]  (optional, can use files.json)
        }

        Args:
            issue_path: Path to the issue directory
            base_prompt: Original prompt from dataset
            target_file: Target file to generate
            context_data: Context dictionary
        """
        # Create task with file-specific instruction
        task = base_prompt
        task += f"\n\nIMPORTANT: Generate ONLY the file '{target_file}'. Output the complete SystemVerilog code for this file."
        ...
        prompt_data = {
            "task": task,
            # doc and files will be read from spec.txt and files.json by entrypoint
        }
        ...
        print(f"Created prompt.json for target: {target_file}")
    ...
    def _generate_volume_changes_patch(
        self, workspace_volume: str, before_volume: str, issue_path: str
    ):
        """
        ...
        try:
            ...
            diff_cmd = [
                ...
                "--rm",
                "-v",
                f"{before_volume}:/before:ro",
                "-v",
                f"{workspace_volume}:/after:ro",
                "ubuntu:22.04",
                "bash",
                "-c",
    ...
    def _create_before_snapshot_volume(self, workspace_volume: str, issue_path: str):
        ...
        try:
            ...
            snapshot_cmd = [
                ...
                "--rm",
                "-v",
                f"{workspace_volume}:/source:ro",
                "-v",
                f"{before_volume}:/target",
                "ubuntu:22.04",
                "bash",
                "-c",
    ...
    def create_repo(self, id: str, model: OpenAI_Instance = None):
        ...
        commit_sha = getattr(self, "commit_hash", None) or ctx.get("context", {}).get(
            "commit"
        )
        data_point = id.split("_")
        ...
        try:
            if (
                not os.getenv("CLONE_HTTP") and "github.com/" in repo_url
                if repo_url
                else False
            ):
        ...
        if repo_url and commit_sha:
            print(
                f"[DEBUG:create_repo-Agentic] Starting datapoint preparation for id={id}"
            )
            ...
            root_dir = (
                "external" if "cvdp_" in repo_url or "cvdp-" in repo_url else None
            )
            ...
            if not success:
                print(
                    f"[ERROR] Failed to create git workspace for {id}, falling back to regular mode"
                )
                ...
                def cleanup_volume():
                    try:
                        ...
                        print(
                            f"[WARNING] Failed to cleanup workspace volume {volume_name}: {e}"
                        )
                ...
                if not hasattr(atexit, cleanup_attr):
                    ...
                    print(
                        f"[INFO] Registered automatic cleanup for volume: {volume_name}"
                    )
